                                  ...
                                  const romea::SkidSteeringCommandLimits & userLimits)
{
  for (size_t i=0; i < 21;i++)
  {
    ...
    for (size_t j=0; j < 21; j++)
    {
      ...
      ASSERT_LE(std::abs(clampedCommandFrame.angularSpeed),
      ...
      ASSERT_LE(std::abs(odometryFrame.leftWheelLinearSpeed),
                parameters.maximalWheelLinearSpeed);
      ASSERT_LE(std::abs(odometryFrame.rightWheelLinearSpeed),
                parameters.maximalWheelLinearSpeed);
      ...
      {
        ASSERT_EQ(romea::near(std::abs(odometryFrame.leftWheelLinearSpeed),
                              parameters.maximalWheelLinearSpeed, 0.001) ||
                  romea::near(std::abs(odometryFrame.rightWheelLinearSpeed),
      ...
      ASSERT_NEAR(clampedCommandFrame.angularSpeed,
    ...
    for (size_t j=0;j < 21;j++)
    {
      ...
      for (size_t k=0; k < 21; k++)
      {
        ...
        for (size_t l=0; l < 21;l++)
        {
          ...
          ASSERT_LE(secondClampedCommandFrame.longitudinalSpeed,
                    userLimits.longitudinalSpeed.upper());
          ASSERT_GE(secondClampedCommandFrame.longitudinalSpeed,
                    userLimits.longitudinalSpeed.lower());
          ASSERT_LE(std::abs(secondClampedCommandFrame.angularSpeed),
                    userLimits.angularSpeed.upper());
          ...
          ASSERT_LE(std::abs((secondOdometryFrame.leftWheelLinearSpeed-firstOdometryFrame.leftWheelLinearSpeed)),
                    parameters.maximalWheelLinearAcceleration*dt+0.000001);
          ASSERT_LE(std::abs((secondOdometryFrame.rightWheelLinearSpeed-firstOdometryFrame.rightWheelLinearSpeed)),
                    parameters.maximalWheelLinearAcceleration*dt+0.000001);
        }
      }
    }
