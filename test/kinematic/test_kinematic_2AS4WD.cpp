  ...
  for (size_t i = 0; i < 21; i++)
  {
    ...
    for (size_t j = 0; j < 21; j++)
    {
      ...
      for (size_t k = 0; k < 21; k++)
      {
        ...
        if (std::abs(clampedCommandFrame.frontSteeringAngle -
                     clampedCommandFrame.rearSteeringAngle) <
                      std::numeric_limits<double>::epsilon())
        {
          ASSERT_NEAR(odometryFrame.frontLeftWheelLinearSpeed,
                      clampedCommandFrame.longitudinalSpeed/
                      std::cos(clampedCommandFrame.frontSteeringAngle), 0.001);
          ASSERT_NEAR(odometryFrame.frontRightWheelLinearSpeed,
                      clampedCommandFrame.longitudinalSpeed/
                      std::cos(clampedCommandFrame.frontSteeringAngle), 0.001);
          ASSERT_NEAR(odometryFrame.rearLeftWheelLinearSpeed,
                      clampedCommandFrame.longitudinalSpeed/
        ...
        if (userLimits.longitudinalSpeed.upper() >= std::numeric_limits<double>::max() &&
            ...
            std::numeric_limits<double>::epsilon())
        {
          ASSERT_EQ(romea::near(std::abs(odometryFrame.frontLeftWheelLinearSpeed),
                    parameters.frontMaximalWheelLinearSpeed, 0.001) ||
                    romea::near(std::abs(odometryFrame.frontRightWheelLinearSpeed),
                    parameters.frontMaximalWheelLinearSpeed, 0.001) ||
                    romea::near(std::abs(odometryFrame.rearLeftWheelLinearSpeed),
        ...
        ASSERT_LE(std::abs(odometryFrame.frontAxleSteeringAngle),
                  parameters.frontMaximalSteeringAngle);
        ASSERT_LE(std::abs(odometryFrame.rearAxleSteeringAngle),
