...
//--------------------------------------------------------------------------
double OneAxleSteeringKinematic::computeBeta(
...
//--------------------------------------------------------------------------
double OneAxleSteeringKinematic::computeAngularSpeed(
...
//--------------------------------------------------------------------------
double OneAxleSteeringKinematic::computeInstantaneousCurvature(
  const double & tanSteeringAngle,
...
//--------------------------------------------------------------------------
double OneAxleSteeringKinematic::computeSteeringAngle(
  const double & instantaneousCurvature,
  const double & wheelBase)
{
  ...
  const double & halfTrack)
{
  return std::sqrt(
    std::pow(
      1 + instaneousCurvature * halfTrack,
      2.) + tanSteeringAngle * tanSteeringAngle) +
         instaneousCurvature * hubCarrierOffset;
}
...
double OneAxleSteeringKinematic::computeLeftWheelLinearSpeed(
  ...
  const double & halfTrack)
{
  return linearSpeed * computeWheelLinearSpeedRatio(
    -tanSteeringAngle,
    -instaneousCurvature,
    hubCarrierOffset,
    halfTrack);
...
//--------------------------------------------------------------------------
double OneAxleSteeringKinematic::computeLinearSpeed(
  ...
  const double & halfTrack)
{
  double leftWheelSpeedRatio = computeWheelLinearSpeedRatio(
    -tanSteeringAngle,
    -instantaneousCurvature,
    hubCarrierOffset,
    halfTrack);

  double rightWheelSpeedRatio = computeWheelLinearSpeedRatio(
    tanSteeringAngle,
    instantaneousCurvature,
    hubCarrierOffset,
    halfTrack);

  return (leftWheelSpeed / leftWheelSpeedRatio + rightWheelSpeed / rightWheelSpeedRatio) / 2.0;
}


//--------------------------------------------------------------------------
OneAxleSteeringCommand OneAxleSteeringKinematic::clamp(
  const double & wheelbase,
  ...
  const OneAxleSteeringCommand & command)
{
  ...
  double rearRatio = (1 + std::abs(tanSteeringAngle) *
    (rearHalfTrack + 2 * rearHubCarrierOffset) / wheelbase);

  maximalAbsoluteLinearSpeed =
    std::min(maximalAbsoluteLinearSpeed, rearMaximalWheelSpeed / rearRatio);

  double frontRatio = OneAxleSteeringKinematic::
    computeWheelLinearSpeedRatio(
    std::abs(tanSteeringAngle),
    std::abs(instantaneousCurvature),
    frontHubCarrierOffset,
    frontHalfTrack);

  maximalAbsoluteLinearSpeed =
    std::min(maximalAbsoluteLinearSpeed, frontMaximalWheelSpeed / frontRatio);
  ...
  const double & dt)
{
  ...
  maximalAbsoluteLinearAcceleration =
    std::min(maximalAbsoluteLinearAcceleration, maximalWheelAcceleration / rearRatio);
  ...
  maximalAbsoluteLinearAcceleration =
    std::min(maximalAbsoluteLinearAcceleration, maximalWheelAcceleration / frontRatio);
