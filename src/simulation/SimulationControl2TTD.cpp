                                                          ...
                                                          const romea::RotationalMotionState & rearRollerWheelSpinningMotion)
{
  ...
  const double frontRollerWheelLinearSpeed = rollerWheelVirtualRadius*frontRollerWheelSpinningMotion.velocity;
  const double rearRollerWheelLinearSpeed = rollerWheelVirtualRadius*rearRollerWheelSpinningMotion.velocity;
...
std::ostream & operator<<(std::ostream &os, const SimulationCommand2TTD & command)
{
  ...
  os << " front left roller wheel spinning setpoint : " << command.frontLeftRollerWheelSpinningSetPoint << std::endl;
  os << " front right roller wheel spinning setpoint : " <<command.frontRightRollerWheelSpinningSetPoint << std::endl;
  os << " rear left roller wheel spinning setpoint : " << command.rearLeftRollerWheelSpinningSetPoint << std::endl;
  os << " rearright roller wheel spinning setpoint : " <<command.rearRightRollerWheelSpinningSetPoint << std::endl;
  return os;
}
...
std::ostream & operator<<(std::ostream &os, const SimulationState2TTD & state)
{
  ...
  os << " right idler wheel spinning motion : " << state.rightIdlerWheelSpinningMotion << std::endl;
  os << " front left  roller wheel spinning motion: " << state.frontLeftRollerWheelSpinningMotion << std::endl;
  os << " front right roller wheel spinning motion : " << state.frontRightRollerWheelSpinningMotion << std::endl;
  os << " rear left idler roller wheel spinning motion: " << state.rearLeftRollerWheelSpinningMotion << std::endl;
  os << " rear right idler roller wheel spinning motion : " << state.rearRightRollerWheelSpinningMotion ;
  return os;
}
                                              ...
                                              const HardwareCommand2TD & hardwareCommand)
{
  ...
  const double idler_ratio = (sprocketWheelRadius+trackThickness)/(idlerWheelRadius+trackThickness);
  const double roller_ratio = (sprocketWheelRadius+trackThickness)/(rollerWheelRadius+trackThickness);

  std::cout <<" ratio" << idler_ratio <<" "<< roller_ratio<< std::endl;
